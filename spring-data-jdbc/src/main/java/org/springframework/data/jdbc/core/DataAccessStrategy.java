...
public interface DataAccessStrategy {
	 ...
	 * @deprecated use {@link #insert(Object, PersistentPropertyPath, EffectiveParentId)} instead.
	 */
	@Deprecated
	@Nullable
	...
	/**
	 * Inserts the data of a single entity. For this the instance is considered to be reachable from the aggregate root via the path given.
	 *
	 * @param <T> the type of the entity
	 * @param instance the entity to be persisted. Must not be {@code null}.
	 * @param path the path by which the instance is reachable from the aggregate root.
	 * @param effectiveParentId the effective id of the parent entity, including ids that might not part of the entity but present in the data model.
	 * @return the id generated for the instance, if any.
	 */
	@Nullable
	default <T> Object insert(T instance, @Nullable PersistentPropertyPath<RelationalPersistentProperty> path,
							  EffectiveParentId effectiveParentId) {
		return insert(instance, (Class<T>)instance.getClass(), effectiveParentId.toParameterMap(path));
	}
	 ...
	 * @deprecated Use {@link #findAllByProperty(PersistentPropertyPath, Object, Object...)}.
	 */
	@Deprecated
	<T> Iterable<T> findAllByProperty(Object rootId, RelationalPersistentProperty property);

	/**
	 * Finds all entities reachable via {@code path} from the instance identified by {@code relativeRootId} using
	 * {@literal keys} as the keys for any {@code Map}s or {@code List}s involved in the path.
	 * 
	 * @param path the path from an entity with an id, to the property to be loaded. May be {@code null}.
	 * @param relativeRootId the id of the entity from which the path starts. May be {@code null}.
	 * @param keys the keys of any {@code Map}s or {@code List}s involved in the path. May be {@code null}.
	 * @param <T> the type of entity to be returned.
	 * @return the loaded entities in an {@code Iterable}. Guaranteed to be not {@code null}.
	 * @since 1.1.0
	 */
	<T> Iterable<T> findAllByProperty(PersistentPropertyPath<RelationalPersistentProperty> path, Object relativeRootId,
			Object... keys);
