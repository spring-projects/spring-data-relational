...
class JdbcQueryCreator extends RelationalQueryCreator<ParametrizedQuery> {
	...
	Join getJoin(SqlContext sqlContext, AggregatePath path) {
		...
		List<Column> idColumns = idDefiningParentPath.getTableInfo().idColumnInfos().toList(ci -> parentTable.column(ci.name()));
		return new Join( //
				currentTable, //
				reverseColumns, //
				idColumns //
	...
	private record Join(Table joinTable, List<Column> joinColumns, List<Column> parentId) {
		...
		Join {
			Assert.isTrue(joinColumns.size() == parentId.size(), "Both sides of a join condition must have the same number of columns");
		}
