...
import org.springframework.data.relational.core.mapping.AggregatePathComparator;
...
class SqlGenerator {
	 ...
	 * @param multiIdCondition
	 * @param columns
	 * @return the IN condition
	 */
	private Condition getSubselectCondition(AggregatePath path,
			Function<Map<AggregatePath, Column>, Condition> multiIdCondition, Map<AggregatePath, Column> columns) {

		AggregatePath parentPath = path.getParentPath();

		if (!parentPath.hasIdProperty()) {
			if (isDeeplyNested(parentPath)) {
				return getSubselectCondition(parentPath, multiIdCondition, columns);
			}
			return multiIdCondition.apply(columns);
		}
		...
		Map<AggregatePath, Column> selectFilterColumns = new TreeMap<>(AggregatePathComparator.INSTANCE);
		...
		if (isFirstNonRoot(parentPath)) { // if the parent is the root of the path
			...
			innerCondition = multiIdCondition.apply(selectFilterColumns);
		} else {

			// otherwise, we need another layer of subselect
			innerCondition = getSubselectCondition(parentPath, multiIdCondition, selectFilterColumns);
		}
	...
	private Expression toExpression(Map<AggregatePath, Column> columnsMap) {
		...
		if (columnsMap.size() == 1) {
			return columnsMap.values().iterator().next();
		} else {
			return TupleExpression.create(new ArrayList<>(columnsMap.values()));
		}
	}
	...
	String createDeleteAllSql(@Nullable PersistentPropertyPath<RelationalPersistentProperty> path) {
		...
		return createDeleteByPathAndCriteria(mappingContext.getAggregatePath(path), isNotNullCondition());
	}
	...
	private Function<Map<AggregatePath, Column>, Condition> inCondition() {
		...
		return columnInfos -> {
			...
			List<Column> columns = List.copyOf(columnInfos.values());
	...
	private Function<Map<AggregatePath, Column>, Condition> equalityCondition() {
		...
		return columnInfos -> {
			...
			for (Map.Entry<AggregatePath, Column> entry : columnInfos.entrySet()) {
	...
	private Function<Map<AggregatePath, Column>, Condition> isNotNullCondition() {
		...
		return columnInfos -> {
			...
			Condition result = null;
			for (Column column : columnInfos.values()) {
		...
		for (Join join : joinTables) {
			...
			for (Pair<Column, Column> columnPair : join.columns) {
				...
				if (condition == null) {
					...
					condition = condition.and(elementalCondition);
				}
			}
	...
	private String createDeleteByPathAndCriteria(AggregatePath path,
		...
		Map<AggregatePath, Column> columns = new TreeMap<>(AggregatePathComparator.INSTANCE);
	...
	record Join(Table joinTable, List<Pair<Column, Column>> columns) {
		Join(Table joinTable, Column joinColumn, Column parentId) {
			this(joinTable, List.of(Pair.of(joinColumn, parentId)));
		}

		Column getJoinColumn() {
			return this.columns.get(0).getFirst();
		}

		@Deprecated
		Column getParentId() {
			return this.columns.get(0).getSecond();
		}
	}
