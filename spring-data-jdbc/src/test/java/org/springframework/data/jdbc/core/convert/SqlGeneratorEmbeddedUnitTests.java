...
class SqlGeneratorEmbeddedUnitTests {
	...
	void deleteByPathEmbeddedId() {
		...
		SqlGenerator sqlGenerator = createSqlGenerator(DummyEntityWithEmbeddedId.class);
		PersistentPropertyPath<RelationalPersistentProperty> path = PersistentPropertyPathTestUtils.getPath("other", DummyEntityWithEmbeddedIdAndReference.class, context);
	...
	void deleteInByPathEmbeddedId() {
		...
		assertSoftly(softly -> {
			...
			softly.assertThat(sql).startsWith("DELETE FROM other_entity WHERE") //
					.contains(" WHERE ") //
					.contains("(other_entity.dummy_entity_with_embedded_id_and_reference_one, other_entity.dummy_entity_with_embedded_id_and_reference_two) IN (:ids)");
		});
	}
	...
	void findOneWithReference() {
		...
		assertSoftly(softly -> {
			...
			softly.assertThat(sql).startsWith("SELECT") //
					...
					.contains(" ON ") //
					.contains(" other.dummy_entity_with_embedded_id_and_reference_one = dummy_entity_with_embedded_id_and_reference.one ") //
					.contains(" other.dummy_entity_with_embedded_id_and_reference_two = dummy_entity_with_embedded_id_and_reference.two ") //
					.contains(" WHERE ") //
	...
	void joinForEmbeddedWithReference() {
		...
		assertSoftly(softly -> {
			...
			softly.assertThat(join.getJoinColumn().getTable()).isEqualTo(join.joinTable());
