...
You can control this behavior by setting `R2dbcMappingContext.setForceQuote(false)`, or use `R2dbcMappingContext.forPlainIdentifiers()` to create a context with plain (unquoted) identifiers.

* Arbitrarily nested objects are not supported by convention.
Use `@Embedded` for value objects that map to columns in the same table — see <<entity-persistence.embedded-entities>>.
...
By default, table and column names derived from classes and properties are used with database-specific quotes, meaning they are typically case-sensitive and reserved SQL words (such as `order`) can be used safely in names.
To disable quoting, override `r2dbcMappingContext(Optional<NamingStrategy>)` in `AbstractR2dbcConfiguration` and return `R2dbcMappingContext.forPlainIdentifiers(…)`, or call `context.setForceQuote(false)` on the context.
When quoting is disabled, Spring Data converts identifier casing to the form used by the configured database without quoting.
For databases that adhere to the SQL standard, names are converted to upper case, so you can use unquoted names when creating tables as long as you do not use keywords or special characters.
...
You can add additional converters to the converter by overriding the `getCustomConverters` method.
...
You can use the `getCustomConverters` method in `AbstractR2dbcConfiguration` to configure converters.
...
To retain the actual enum value, register a `@WritingConverter` whose source and target types use the actual enum type to avoid using `Enum.name()` conversion.
