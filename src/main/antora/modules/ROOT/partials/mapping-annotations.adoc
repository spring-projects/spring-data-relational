...
Names specified with a `@Column` annotation are quoted when `forceQuote` is enabled (the default for JDBC; disabled by default for R2DBC).
For most databases, quoting means that these names are case-sensitive.
...
A value of `null` for wrapper types (or `zero` for primitive types) marks the entity as new.
The initially stored value is `zero` for wrapper types (`one` for primitive types).
