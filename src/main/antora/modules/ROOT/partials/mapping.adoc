...
When the column naming strategy does not match your database column names, you can override the column name with the javadoc:org.springframework.data.relational.core.mapping.Column[] annotation.
...
annotation can be used on Sets, Lists, and Maps (one-to-many relationships).
The `idColumn` element of the annotation provides a custom name for the foreign key column referencing the id column in the other table.
It also customizes the back-reference column name for single-entity (one-to-one) references.
